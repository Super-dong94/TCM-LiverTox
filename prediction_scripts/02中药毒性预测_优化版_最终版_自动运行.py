# -*- coding: utf-8 -*-
# 本文件由 02中药毒性预测_优化版_最终版(2).ipynb 自动转换生成。
# 代码单元内容按 notebook 原顺序保留，未修改核心代码。

# %% [markdown]
# # 中药毒性预测流程（优化版，CSV 精简输出）
#
# 本 notebook 参考 `01方剂毒性预测.ipynb` 原始版与 `01方剂毒性预测_优化版.ipynb` 的整理规则，对 `02中药毒性预测.ipynb` 进行结构化优化。
#
# 本版优化原则：
#
# 1. **主要结果统一保存为 CSV**，不再保存 Excel、JSON、TXT；
# 2. **不保存分块中间文件**，不会再生成分块中间文件；
...
# | 文件 | 内容 |
# |---|---|
# | `01_chinese_medicine_related_formulae.csv` | 含有目标中药的方剂记录 |
# | `02_chinese_medicine_compound_master.csv` | 中药-化合物-CID-SMILES-化学分类整合主表 |
...
# 0. 基础配置
# ============================================================
# 这里统一设置待预测中药、入血成分参考库路径、输出目录、分块大小和毒性阈值。
# 后续如果需要更换中药或路径，优先修改本单元即可。
...
# -----------------------------
# 支持一个或多个中药名称。
# 例如：CHINESE_MEDICINES = ["人参"]
# 例如：CHINESE_MEDICINES = ["人参", "白术", "茯苓", "甘草"]
CHINESE_MEDICINES = _json_string_list_env("TOXHERB_CHINESE_MEDICINES_JSON", ["人参", "白术", "茯苓", "甘草"])

# 将中药列表合并成一个字符串，用于输出目录和摘要记录。
CHINESE_MEDICINE_GROUP_NAME = "_".join(CHINESE_MEDICINES[:2])
...
def safe_filename(text: str) -> str:
    ...
    return text or "chinese_medicine"
...
OUT_DIR = OUTPUT_BASE_DIR / f"{safe_filename(CHINESE_MEDICINE_GROUP_NAME)}_{RUN_ID}"
...
print(f"当前分析中药：{CHINESE_MEDICINE_GROUP_NAME}")
...
toxherb_progress("entity_resolution", 8, "中药实体解析与数据表检查")
...
# %% [markdown]
# ## 4. 中药 → 相关方剂 → 化合物 → SMILES 整合
#
# 这一节完成两个核心任务：
#
# 1. 查询含有目标中药的方剂，保存为 `01_chinese_medicine_related_formulae.csv`；
# 2. 查询目标中药的化学成分并合并 SMILES、化学分类等信息，保存为 `02_chinese_medicine_compound_master.csv`。
#
# 说明：
#
# - 原始版中使用中药名去匹配 `Formula.Chinese.name`，这容易漏掉很多方剂；
# - 本版改为使用 `Herb.Chinese.name` 查询含有目标中药的方剂；
...
# 4.1 查询含有目标中药的方剂
# ============================================================
# 逻辑：data_01 是方剂-中药组成表，因此应使用 Herb.Chinese.name 匹配目标中药，
# 而不是用中药名去匹配 Formula.Chinese.name。

formulae_df = (
    data_01
    .loc[data_01["Herb.Chinese.name"].isin(CHINESE_MEDICINES)]
    .drop_duplicates()
    .reset_index(drop=True)
)

if formulae_df.empty:
    print(f"警告：data_01 中没有检索到含有这些中药的方剂：{CHINESE_MEDICINES}")
else:
    # 给每个方剂增加命中中药数量，便于后续筛选“同时包含多味目标中药”的方剂
    matched_count_df = (
        formulae_df
        .groupby("Formula.Chinese.name")["Herb.Chinese.name"]
        .nunique()
        .reset_index(name="Matched_Herb_Count")
    )

    formulae_df = (
        formulae_df
        .merge(matched_count_df, on="Formula.Chinese.name", how="left")
        .sort_values(["Matched_Herb_Count", "Formula.Chinese.name"], ascending=[False, True])
        .reset_index(drop=True)
    )

show_and_save_csv(formulae_df, "01_chinese_medicine_related_formulae.csv")

print(f"目标中药数量：{len(CHINESE_MEDICINES)}")
print(f"相关方剂记录数：{len(formulae_df)}")
print(f"相关唯一方剂数量：{formulae_df['Formula.Chinese.name'].nunique() if not formulae_df.empty else 0}")

# %%
# ============================================================
# 4.2 构建中药-化合物-SMILES 整合主表
# ============================================================

# 1）筛选目标中药对应的化学成分
herb_compound_df = (
    data_02
    .loc[data_02["Herb.Chinese.name"].isin(CHINESE_MEDICINES)]
    .drop_duplicates()
    .reset_index(drop=True)
)

if herb_compound_df.empty:
    raise ValueError(
        f"没有在 data_02 中检索到中药：{CHINESE_MEDICINES}\n"
        "请检查中药中文名称是否与数据库完全一致。"
    )
...
# 4）增加分析对象名称，便于多个中药组合分析时回溯
compound_master_df.insert(0, "Chinese_medicine_group", CHINESE_MEDICINE_GROUP_NAME)
...
# 7）实际用于入血预测的唯一 SMILES 输入表；该表仅作为内存变量，不单独写出
prediction_input_df = (
    compound_master_df
...
# 8）只保存一个整合主表，不再保存多个中间表
show_and_save_csv(compound_master_df, "02_chinese_medicine_compound_master.csv")
toxherb_progress("entity_resolution", 20, "中药-成分映射完成")
...
# 本节只输出一个正式结果文件：
#
# - `03_intoblood_prediction_results.csv`：全部候选化合物的入血预测结果，已合并中药、化合物和 SMILES 来源信息。
#
# 重要说明：
#
# - 本版**不会保存任何分块 分块文件**；
# - 本版改为每批结果进入内存列表，最后 `pd.concat()` 合并；
...
def predict_intoblood_in_chunks(
    ...
    本函数与原始版相比最大的区别：
    - 原始版：每批保存为临时文件，然后再读取合并；
    - 本版：每批结果只加入内存列表 part_results，最后一次性 concat 合并；
    - 因此不会在工作目录产生任何 分块中间文件。
...
# %%
# ============================================================
# 6.2 生成并保存中药层面的综合评估摘要
# ============================================================
# 本优化版只保存 CSV，不再额外保存 JSON 和 TXT。

summary_df = pd.DataFrame([{
    "Chinese_medicines": CHINESE_MEDICINE_GROUP_NAME,
...
show_and_save_csv(summary_df, "05_chinese_medicine_toxicity_summary.csv")

print("\n" + "=" * 60)
print("📊 中药肝毒性系统生物学评估报告")
print("=" * 60)
...
#
# 原始版中分散的 `Class`、`Superclass`、`Pathway`、`Is_glycoside` 统计不再分别输出，而是统一进入后续的 `11_consolidated_count_summary.csv`。
...
toxherb_progress("complete", 100, "中药预测报告生成完成")
