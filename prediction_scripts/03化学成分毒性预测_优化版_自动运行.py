...
summary_df = pd.DataFrame([{
    ...
    "Unique_SMILES_for_prediction": len(prediction_input_df),
...
# ## 7. 高危肝毒性成分筛选
#
# 这一节筛选 `Max_Tox_Prob >= TOXIC_THRESHOLD` 的高危肝毒性成分，并合并 CID、来源中药、化学分类等信息。
#
# 输出：
#
...
if high_toxic_df.empty:
    target_df = pd.DataFrame(columns=["ChemicalName", "CID", SMILES_COL, "Symbol"])
else:
    compound_structure_map_df = (
        data_03_work
        .rename(columns={"SMILES": SMILES_COL})
        [[c for c in ["CID", SMILES_COL] if c in data_03_work.rename(columns={"SMILES": SMILES_COL}).columns]]
        .drop_duplicates()
    )
    ...
    target_df = (
        ...
        .merge(data_02_work[["ChemicalName", "CID"]].drop_duplicates(), on="CID", how="left")
    ...
    # 补充来源中药信息
    source_cols = compact_metadata_cols(high_toxic_df, [SMILES_COL, "Source_Herbs", "Source_Herb_Count"])
...
print(f"高危成分关联靶标数量：{target_df['Symbol'].nunique() if not target_df.empty and 'Symbol' in target_df.columns else 0}")
...
# 9.1 构建统一统计汇总表
# ============================================================
# 将高危成分化学类别、来源中药、靶标、通路、GO、疾病统计结果整合到一个 CSV。
